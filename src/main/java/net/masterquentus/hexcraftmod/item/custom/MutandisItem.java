...
public class MutandisItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if(validTag.size() == 0 || validTag.stream().allMatch(blacklistTag::contains)) { // This check prevents the while loop below from becoming infinite.
            HexcraftMod.LOG.error("Mutandis tag is invalid! This means the tag is empty, or every item in it is blacklisted.");
        ...
        if(!state.is(blacklistTag) && state.is(validTag)) {
            ...
            if(!level.isClientSide) {
                ...
                while(newState == null) {
                    ...
                    BlockState _state = validTag.getRandomElement(HexcraftMod.RANDOMSOURCE).orElse(null).value().defaultBlockState();
